...
#import "MarriagePickerView.h"

@interface MarriagePickerView () <UIPickerViewDataSource,UIPickerViewDelegate>
...
@implementation MarriagePickerView
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _dataSource = [NSMutableArray arrayWithObjects: @"未婚",@"已婚", nil];
        _selectSex = @"未婚";
        _selectIndex = 0;
        ...
        UIBarButtonItem *confirmBarButtonItem = [[UIBarButtonItem alloc] bk_initWithTitle:@"确定" style:UIBarButtonItemStylePlain handler:^(id sender) {
            STRONG_SELF(self);
            [self hide];
            if (self.delegate && [self.delegate respondsToSelector:@selector(marriagePickerView:didSelectRow:titleForRow:)]) {
                [self.delegate marriagePickerView:self didSelectRow:self.selectIndex titleForRow:self.selectSex];
            }
        }];
