...
- (void)drawRect:(CGRect)rect {
    ...
    slider = [[JXCircleSlider alloc] initWithFrame:self.bounds];
    ...
    CGFloat textFieldW = self.frame.size.width/1.75;
...
-(void)newValue:(JXCircleSlider*)view
{
    ...
    _angle = value/14.4;
    _textField.text = [NSString stringWithFormat:@"%.1f",_angle];
    [self setGradientColorValue:_angle type: _bloodSugarType];
}
...
- (void)setBloodSugarType:(EBloodSugarType)bloodSugarType
{
    _bloodSugarType = bloodSugarType;
    [self setGradientColorValue:_angle type: _bloodSugarType];
}

- (void)setGradientColorValue:(CGFloat)angle type:(EBloodSugarType)type
{    
    EBloodSugarDiagnosisType diagnosisType = [BloodSugarJudgment diagnosisFromBloodSugarType:type bloodSugarValue:_angle];
    //BloodSugarJudgment
    if (diagnosisType == 0) {
        gradientLayer.colors = @[(__bridge id)RGB(254, 112, 167).CGColor,(__bridge id)RGB(253, 80, 137).CGColor];
    }
    else if (diagnosisType == 1)
    {
