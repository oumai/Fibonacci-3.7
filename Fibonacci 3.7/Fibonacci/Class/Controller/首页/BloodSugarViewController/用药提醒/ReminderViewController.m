...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(TextViewEditChanged:)
                                               ...
                                               object:noteView];
    [self addRightButton:@selector(saveReminderData) titile:@"保存" titleColor:RGB(77, 77, 79)];
...
- (void)initTableViewData
{
    ...
    myTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, MainScreenWidth, MainScreenHeight-kStatusAndNavHeight)];
...
- (void)initTableViewFootView
{
    ...
    deleteButton.frame = CGRectMake(40, CELLHEIGHT*0.5, MainScreenWidth - 100, CELLHEIGHT);
    [deleteButton.titleLabel setFont:[UIFont boldSystemFontOfSize:15.0]];
    [deleteButton setTitle: @"删除" forState:UIControlStateNormal];
    deleteButton.layer.cornerRadius = CELLHEIGHT/2;
    deleteButton.layer.borderColor = RGB(153, 153, 153).CGColor;
...
- (void)pagesLayout
{
    ...
    label.textColor = RGB(193, 193, 194);
    ...
    CGFloat buttonW = (MainScreenWidth - 20*4)/3;
    weekView = [[UIView alloc] init];
    weekView.backgroundColor = RGB(242, 243, 244);
    ...
    [array enumerateObjectsUsingBlock:^(NSObject *obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        buttonX = 20 + (idx%3)*(buttonW + 20);
        buttonY = 10 + (idx/3)*(buttonH + 10);
    ...
    UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
    button.frame = CGRectMake(0, maxY, MainScreenWidth, CELLHEIGHT);
...
- (void)viewDidAppearLoadData
{
    ...
    birthDayPickerView = [[BATBirthDayPickerView alloc] initWithFrame:CGRectMake(0, MainScreenHeight - kStatusAndNavHeight, MainScreenWidth, 256) type:EBirthDayTypeDateWeek];
    birthDayPickerView.delegate = self;
    [self.view addSubview: birthDayPickerView];
    
    insulinPickerView = [[BATInsulinPickerView alloc] initWithFrame:CGRectMake(0, MainScreenHeight - kStatusAndNavHeight, MainScreenWidth, 256)];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath;
{
    ...
    if (cell == nil) {
        ...
        titlelabel.tag = 100;
        titlelabel.textColor = RGB(78, 79, 81);
        ...
        rigthLabel.textColor = RGB(153, 153, 153);
        ...
        lineView.tag = 99;
        lineView.hidden = YES;
        lineView.backgroundColor = RGB(231, 232, 233);
    ...
    if ([tableView isEqual:myTableView])
    {
        ...
        switch (indexPath.row) {
            ...
            case 7:
            {
                label.textColor = RGB(143, 144, 145);
                cell.backgroundColor = RGB(233, 234, 235);
            }
                break;
            case 1:
            {
