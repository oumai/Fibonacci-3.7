...
- (void)viewDidLoad {
    ...
    {
        [self addRightButton:@selector(saveBloodSugarData) titile:@"保存" titleColor:RGB(77, 77, 79)];
    }
    [self pagesLayout];
...
-(void)pagesLayout
{
    ...
    NSInteger type = [segmentTitleArray count];
    if (isPreview) {
        type = [_previewModel.timeScale integerValue];
    }
    ...
    for (int i = 0; i < [segmentTitleArray count]; i++)
    {
        ...
        if (isPreview)
        {
            ...
            {
                [button setTitleColor:RGB(49, 50, 51) forState:UIControlStateNormal];
            }
            if (type >6) {
    ...
    lineLayer.backgroundColor = [AppColor CGColor];
    ...
    bloodSugarSlider.backgroundColor = [UIColor whiteColor];
    bloodSugarSlider.tag = 1001;
    bloodSugarSlider.bloodSugarType = type;
...
-(void)segmentAction:(UIButton *)button
{
    for (UIView *elem in segmentScrollView.subviews) {
        if ([elem isKindOfClass:[UIButton class]])
        {
            ...
            [elemButton setTitleColor: RGB(77, 77, 79) forState:UIControlStateNormal];
            button.selected = NO;
        }
    }
    button.selected = YES;
    ...
    bloodSugarSlider.bloodSugarType = button.tag;
    timeScale = nil;
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        if (result) {
            ...
            dispatch_after(time, dispatch_get_main_queue(), ^{
                [BloodSugarValueHUD presentViewFromValue:bloodSugarSlider.angle type:bloodSugarSlider.bloodSugarType];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath;
{
    ...
    if (cell == nil) {
        ...
        cell.textLabel.textColor = RGB(151, 151, 151);
    }
