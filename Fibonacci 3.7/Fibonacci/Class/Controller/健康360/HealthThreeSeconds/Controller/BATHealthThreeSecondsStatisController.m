...
- (void)setupUIWith:(NSArray *)items {
    ...
    for (NSInteger i = 0; i < items.count; i++) {
        ...
        [button setBackgroundImage:[self buttonBackBGColorWith:@[AppColor,AppColor]] forState:UIControlStateSelected];
...
- (void)statisticsDataRequestWith:(BATRequestType)type {
    ...
    [HTTPTool requestWithURLString:@"/api/EatCircle/HealthDataStatistics" parameters:params showStatus:YES type:kGET success:^(id responseObject) {
...
- (BATStatisSegmentControl *)segmentControl {
    if (!_segmentControl) {
        _segmentControl = [[BATStatisSegmentControl alloc] initWithFrame:CGRectMake(20, 20, MainScreenWidth-20*2, 35) itemsName:@[@"日",@"周",@"月"]];
