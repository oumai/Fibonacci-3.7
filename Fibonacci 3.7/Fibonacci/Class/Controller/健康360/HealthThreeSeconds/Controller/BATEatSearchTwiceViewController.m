...
static NSString *const studioListCellId = @"BATEatSearchTableViewCell.h";
...
//            self.alertViewOfME.hidden = YES;
    ...
    [self.alertView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.equalTo(appDelegate.window.mas_top).offset(MainScreenHeight);
        make.left.right.equalTo(appDelegate.window);
        make.height.mas_equalTo(MainScreenHeight);
    }];
...
- (UIView *)topSearchView{
    ...
    if (!_topSearchView) {
        _topSearchView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, MainScreenWidth, 70)];
        _topSearchView.backgroundColor = [UIColor whiteColor];
        
        UIView *viewTFBgView = [[UIView alloc] init];
        viewTFBgView.frame = CGRectMake(15, 15, MainScreenWidth - 30, 40);
...
- (void)textFieldDidChange:(UITextField *)textFiled{
    ...
    [HTTPTool requestWithURLString:@"/api/EatCircle/GetAssociationFood"  parameters:dictM showStatus:YES type:kGET success:^(id responseObject) {
        BATSearchFoodModel * foodModel = [BATSearchFoodModel mj_objectWithKeyValues:responseObject];
        //这里要改 为找到的食物
        _count = 0;
        //更新table的约束
        [self updateTableInside];
...
- (void)loadSearchResultRequestWith:(NSString *)keyWord{
    ...
    [HTTPTool requestWithURLString:@"/api/EatCircle/GetRecommendFoodList" parameters:dictM showStatus:YES type:kGET success:^(id responseObject) {
        BATSearchFoodModel *foodlistModel = [BATSearchFoodModel mj_objectWithKeyValues:responseObject];
        if ([foodlistModel.ResultMessage isEqualToString:@"暂无数据！"]) {
            //这个时候的数据用count判读
            _count = 1;
        ...
        [self.dataSource removeAllObjects];
        for (NSDictionary *dic in foodlistModel.Data) {
            BATHealthThreeSecondsRecommedFoodListModel *model = [BATHealthThreeSecondsRecommedFoodListModel mj_objectWithKeyValues:dic];
...
- (UITextField *)navTextField{
    if (!_navTextField) {
        _navTextField = [[UITextField alloc]initWithFrame:CGRectMake(0, 0, MainScreenWidth  , 40)];
...
- (UIView *)alertViewOfME {
    ...
    if (!_alertViewOfME) {
        ...
        _alertViewOfME.frame = CGRectMake(0, 0, MainScreenWidth, 50);
        UILabel *label = [[UILabel alloc] init];
        label.frame = CGRectMake(10, 0, MainScreenWidth - 20, 40);
...
- (BATHealthThreeSecondsFoodEnterAlertView *)foodEnterAlertView{
    if (!_foodEnterAlertView) {
        _foodEnterAlertView = [[BATHealthThreeSecondsFoodEnterAlertView alloc]initWithFrame:CGRectMake(0, 0, MainScreenWidth, MainScreenHeight)];
...
- (void)addFoodDataRequest{
    ...
    WeakSelf;
    [HTTPTool requestWithURLString:@"/api/EatCircle/AddDietDetails" parameters:dictM0 showStatus:YES type:kPOST success:^(id responseObject) {
...
- (UISearchBar *)searchBar {
    if (!_searchBar) {
        ...
        _searchBar.frame = CGRectMake(15, 0, MainScreenWidth - 30, 40);
...
- (void)showAlertViewWithString:(NSString *)str {
    ...
    [alertController addAction:OKAction];
     ...
     AppDelegate *appDelegate = (AppDelegate *)[UIApplication sharedApplication].delegate;
    [appDelegate.rootViewController presentViewController:alertController animated:YES completion:nil];
