...
- (void)judgeLoginStation
{
    ...
    {
        if (![self.title isEqualToString:@"健康3秒钟"])
        {
            [self setupNav];
            [self commonInit];
            [self registerCell];
...
- (void)setupNav{
    ...
    self.title = @"健康3秒钟";
    [self addRightButton:@selector(recordButtonClick) titile:@"统计" titleColor:AppColor];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    else if (indexPath.section == 3)
    {
        ...
        else
        {
            ...
            runRecordCell.stepCountLabel.text = [NSString stringWithFormat:@"%.0f",_stepNum];
            if (_stepNum) {
                runRecordCell.calorieLabel.text = [NSString stringWithFormat:@"%0.2f公里/%0.2f卡路里",_stepNum*0.6*0.001,_stepNum*0.04];
            }else{
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    if (section == 1) {
        UIView *headView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, MainScreenWidth, 55)];
        headView.backgroundColor = [UIColor whiteColor];
        
        UILabel *headLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, 20, MainScreenWidth, 30)];
...
- (void)bottomHealthReportButtonClick{
    ...
    reportVC.Steps = [NSString stringWithFormat:@"%.0f",_stepNum];
...
- (void)choosePhoto{
    ...
    imagePickerVc.photoWidth = MainScreenWidth;
...
- (void)loadDataRequest:(NSString  *)dateStr{
    ...
    WeakSelf
    [HTTPTool requestWithURLString:@"/api/EatCircle/GetHealthData" parameters:dictM showStatus:YES type:kGET success:^(id responseObject) {
...
- (void)updateDataRequestWithDict:(NSMutableDictionary *)dictM
    ...
    [HTTPTool requestWithURLString:@"/api/EatCircle/UpdateHealthData" parameters:dictM showStatus:YES type:kPOST success:^(id responseObject) {
...
- (UIButton *)bottomHealthReportButton{
    if (!_bottomHealthReportButton) {
        ...
        _bottomHealthReportButton.backgroundColor = AppColor;
...
- (UIButton *)tableViewFooterButton{
    if (!_tableViewFooterButton) {
        _tableViewFooterButton = [[UIButton alloc]initWithFrame:CGRectMake(0, 0, MainScreenWidth, 70)];
        ...
        [_tableViewFooterButton setTitleColor: AppColor forState:UIControlStateNormal];
