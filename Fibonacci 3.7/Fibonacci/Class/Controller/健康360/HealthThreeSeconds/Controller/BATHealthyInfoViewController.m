...
static NSString *const DateCellID = @"BATHealthyInfoTableViewCell.h";
...
//        UIColor *navBtnColor = [UIColor gradientFromColor:START_COLOR toColor:END_COLOR withHeight:30];
        ...
        [recordButton setTitleColor:AppColor forState:UIControlStateNormal];
...
//        _tableView.tableFooterView = self.tableViewFooterView;
        ...
        view.frame = CGRectMake(0, 0, 75, MainScreenWidth);
...
- (BATDatePickerView *)datePickerView {
    if (!_datePickerView) {
        _datePickerView = [[BATDatePickerView alloc] initWithFrame:CGRectMake(0, MainScreenHeight - 64, MainScreenWidth, 256)];
...
- (UIButton *)tableViewFooterView{
    if (!_tableViewFooterView) {
        _tableViewFooterView = [[UIButton alloc]initWithFrame:CGRectMake(10, 35, MainScreenWidth - 20, 40)];
        _tableViewFooterView.backgroundColor = AppColor;
...
- (void)personInfoListRequest {
    ...
    [HTTPTool requestWithURLString:@"/api/Patient/Info" parameters:nil showStatus:YES type:kGET success:^(id responseObject) {
