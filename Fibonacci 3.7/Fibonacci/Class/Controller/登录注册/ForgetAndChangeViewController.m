...
- (void)initPagesData
{
    ...
    countdownButton = [UIButton buttonWithType:UIButtonTypeCustom Title:@"获取验证码" titleColor:[UIColor whiteColor] backgroundColor:AppColor backgroundImage:nil Font:[UIFont systemFontOfSize:14]];
...
//    }
    ...
    button.frame = CGRectMake(buttonH/2, 0, MainScreenWidth - buttonH, buttonH);
    ...
    if (_type==0) {
        ...
        registLabel.textColor = RGB(51, 51, 51);
...
{
    [HTTPTool requestWithURLString:[NSString stringWithFormat:@"/api/system/getMessageImage/%@",phoneField.text] parameters:nil showStatus:YES type:kGET success:^(id responseObject) {
        
        NSData *data = [[NSData alloc] initWithData:[GTMBase64 decodeString:[responseObject objectForKey:@"Data"]]];
...
- (void)codeRequest:(NSString *)imageCode  {
    ...
    if (_type == 0)
    {
        url = [NSString stringWithFormat:@"/api/Account/SendVerifyCode/%@/10/%@",phoneField.text,imageCode];
        valid = @"10";
    }
    else
    {
        url = [NSString stringWithFormat:@"/api/Account/SendVerifyCode/%@/20/%@",phoneField.text,imageCode];
        valid = @"10";
    }
    [HTTPTool requestWithURLString:url parameters:nil showStatus:YES type:kGET success:^(id responseObject) {
        ...
        if ([code integerValue] == 0) {
            ...
            [KMTools countdownWithTime:120 End:^{
                ...
                [countdownButton setTitleColor:AppColor forState:UIControlStateNormal];
