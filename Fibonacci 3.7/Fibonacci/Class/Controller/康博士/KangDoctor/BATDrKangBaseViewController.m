...
@property (nonatomic,assign) BOOL loginState; //登录状态是否改变

@property (nonatomic,assign) BOOL firstIn; //登录状态是否改变

//@property (nonatomic,assign) BOOL firstInstructions; //是否第一个指令
@end
...
- (void)viewDidLoad {
    ...
    if ([[NSUserDefaults standardUserDefaults] boolForKey:DEVICE_FIRST_RECORD] != YES) {
        ...
        if (_measureType) {
            _firstInVC = YES;
            NSString *content = @"start";
            switch (_measureType) {
                case EKangMeasureTypeBlood:
                {
                    content = @"血液相关";
                }
                    break;
                case EKangMeasureTypeVitalCapacity:
                {
                    content = @"肺活量测量";
                }
                    break;
                case EKangMeasureTypeMedicallyExamined:
                {
                    content = @"快速体检";
                }
                    break;
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    [self beginText:animated];
    _firstIn = NO;
}

- (void)viewWillDisappear:(BOOL)animated {
    [super viewWillDisappear:animated];
    _pageNavHidden = self.navigationController.navigationBarHidden;
...
- (void)sendMsg:(NSString *)content avatarUrl:(NSString *)avatarUrl {
    ...
    NSString *jsonStr = [KMTools dataTojsonString:dic];
...
- (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType {
    ...
    NSDictionary *urlPara = [KMTools getParametersWithUrlString:tmpUrl];
    ...
    [UIView animateWithDuration:duration delay:duration options:animation animations:^{
        ...
        [self.bottomCollectionView mas_updateConstraints:^(MASConstraintMaker *make) {
            ...
            if(@available(iOS 11.0, *)) {
                ...
                make.bottom.equalTo(@(-keyboardFrame.size.height-49-y));
            }
        }];
        ...
        [self.inputBar mas_updateConstraints:^(MASConstraintMaker *make) {
            ...
            if(@available(iOS 11.0, *)) {
                ...
                make.bottom.equalTo(@(-keyboardFrame.size.height-y));
            }
        }];
...
//            make.bottom.equalTo(self.bottomCollectionView.mas_top).offset(2);
            ...
            else
            {
                if(!_measureType)
                {
                    make.bottom.equalTo(self.bottomCollectionView.mas_top).offset(45);
                }
                else
                {
                    make.bottom.equalTo(self.bottomCollectionView.mas_top).offset(2);
...
- (void)createDatabaseTable {
    ...
    if ([self.historyDB open]) {
        ...
        NSString *tableName = [KMTools getDateStringWithDate:[NSDate date] Format:@"yyyyMMdd"];
...
- (void)beginText:(BOOL)animated
{
    if (!_firstIn && _measureType == 0)
    {
        [self getSelectTabbar];
    }
    if (_selectTabbar && _measureType == 0)
    {
        if (_isNeedLogin) {
            if (LOGIN_STATION)
            {
                [self sendTextRequestWithContent:@"暂不登录" isSound:NO];
            }
        }
        if (_bottomKeyArray.count == 0) {
            [self sendTextRequestWithContent:@"start" isSound:NO];
//            NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
//            [userDefaults setValue:@"" forKey:KEY_KANG_PROPOSALSTRING];
    ...
    else
    {
        if (_gotoStepVC) {
            ...
            if (self.isExecutiveOrder) {
                ...
                self.isExecutiveOrder = NO;
                dispatch_time_t time = dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 *NSEC_PER_SEC));
                dispatch_after(time, dispatch_get_main_queue(), ^{
                    [self.navigationController setNavigationBarHidden:_pageNavHidden animated:animated];
                });
            }
        }
//        else
...
- (void)executiveOrder:(NSString *)order {
    if ([order containsString:@"show"]) {
        //小管家跳转
        _isExecutiveOrder = NO;
        _gotoStepVC = NO;
        [self getProposalStr];
        if (self.proposalStr.length > 0)
        {
            [self sendTextRequestWithContent:self.proposalStr isSound:NO];
            NSArray *array = [self.proposalStr componentsSeparatedByString:@"/"];
            if (array.count > 0)
            {
                for (NSString *elem in array) {
                    [self receiveMsg:elem];
                    [self insertMsgWithContent:elem AndSender:@"DrKang"];
                }
            }
        }
        else
...
- (void)getProposalStr
{
    ...
    if ([allKeys containsObject:KEY_KANG_PROPOSALSTRING]) {
        _proposalStr = [userDefaults valueForKey: KEY_KANG_PROPOSALSTRING];
    }
}

- (void)getSelectTabbar
{
    NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
    NSArray *allKeys = [userDefaults.dictionaryRepresentation allKeys];
    if ([allKeys containsObject:KEY_KANG_SELECTTABBAR]) {
        NSString *str = [userDefaults valueForKey: KEY_KANG_SELECTTABBAR];
        if (str.length > 0)
        {
            _selectTabbar = YES;
        }
    ...
    [userDefaults setValue:@"" forKey:KEY_KANG_SELECTTABBAR];
    [userDefaults synchronize];
}
...
- (void)showWelcomeView {
    ...
    [self.chatWebView mas_updateConstraints:^(MASConstraintMaker *make) {
        if (iPhoneX) {
            make.top.equalTo(@(44+284.0/750.0*MainScreenWidth));
        }
        else {
            make.top.equalTo(@(20+284.0/750.0*MainScreenWidth));
        }
    }];
        ...
        if ([result.msg isEqualToString:@"txt"]) {
            ...
            if (result.resultData.length > 0 && ![result.resultData isEqualToString:@"null"]) {
                ...
                if (isSound) {
                    ...
                    NSString *string = [[self filterHTML:result.resultData] stringByTrimmingCharactersInSet:[NSCharacterSet characterSetWithCharactersInString:@"@／：；（）¥「」＂、[]{}#%-*+=_\\|~＜＞$€^•'@#$%^&*()_+'\""]];
            ...
            if ([result.resultData containsString:@"登录"])
            {
                _isNeedLogin = YES;
            }
            else
...
//        
//        [YCXMenu showMenuInView:self.view fromRect:CGRectMake(MainScreenWidth - 55, 0, 60, 0) menuItems:self.items selected:^(NSInteger index, YCXMenuItem *item) {
//            
...
//    }];
    ...
    [self.topImageView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.height.mas_equalTo(284.0/750.0*MainScreenWidth);
    }];
...
- (UIButton *)navBackButton {
    ...
    if (!_navBackButton) {
        ...
        [_navBackButton setImage:[UIImage imageNamed:@"nav_back_bg"] forState:UIControlStateNormal];
